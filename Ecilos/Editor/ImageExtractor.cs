...
namespace Ecilos.Editor.Image {
  public class ImageExtractor {
    ...
    public static void ExtractAsset(string guid, string extension = "jpg", int quality = 100) {
      string assetPath = AssetDatabase.GUIDToAssetPath(guid);

      AssetImporter assetImporter = AssetImporter.GetAtPath(assetPath);

      IEnumerable<Object> materials = AssetDatabase.LoadAllAssetsAtPath(assetImporter.assetPath)
                                          .ToList()
    ...
    private static void WriteTexture(string assetPath, Texture2D texture, string extension, int quality = 100) {
      byte[] bytes;

      if (extension == "jpg")
        bytes = texture.EncodeToJPG(quality);
      else
        throw new ArgumentException("Only \"jpg\" extension currently supported!");

      File.WriteAllBytes(BuildTexturePath(assetPath, texture.name, extension), bytes);
    }

    public static string BuildTexturePath(string assetPath, string textureName, string extension) {
      string assetFolder = assetPath.Substring(0, assetPath.LastIndexOf('/'));
      string assetPathWithoutExtension = assetPath.Substring(0, assetPath.LastIndexOf('.'));
